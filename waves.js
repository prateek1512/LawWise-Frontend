...
function initWaves(containerId, canvasId, cursorId, configOverrides) {
    ...
    const config = Object.assign({
        lineColor: 'rgba(107, 122, 141, 0.15)',
        waveSpeedX: 0.02,
        waveSpeedY: 0.01,
        waveAmpX: 40,
        waveAmpY: 20,
    ...
    function tick(t) {
        ...
        if (cursor) {
            cursor.style.transform = `translate3d(calc(${mouse.sx}px - 50%), calc(${mouse.sy}px - 50%), 0)`;
        }
...
document.addEventListener("DOMContentLoaded", () => {
    // Hero waves
    initWaves("waves-container", "waves-canvas", "waves-cursor");

    // LIN section waves (softer lines for card background)
    initWaves("lin-waves-container", "lin-waves-canvas", null, {
        lineColor: 'rgba(107, 122, 141, 0.08)',
        waveAmpX: 30,
        waveAmpY: 15,
        xGap: 14,
        yGap: 40
